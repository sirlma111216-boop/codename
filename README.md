...
- 구조: `브라우저 ↔ Cloudflare Worker(API·WebSocket) ↔ 방마다 Durable Object 하나`. 별도 DB 서비스 없이 방 상태를 Cloudflare Durable Object 내장 저장소(SQLite)에 보관합니다.
...
첫 배포에서 `RoomDurableObject` 의 SQLite 마이그레이션(`v1`, `new_sqlite_classes`)이 적용됩니다.
...
src/server/   Worker 라우팅·세션·Origin 검사(index.ts), 방 Durable Object(room.ts), 방·권한 로직(room-logic.ts)
...
tests/        unit(엔진·방), e2e(멀티클라이언트·경쟁·보안·재시작), fixtures(합성 테스트 데이터)
