...
- 코드에도 한도를 태울 수 있는 곳이 있었습니다: 드문 실패 상황(클래스↔방 알림이 거절되거나 계속 실패)에서 방은 3초, 클래스는 최대 0.5초 간격으로 끝없이 재시도했습니다. 같은 날 고쳐, 이제 재시도 간격이 늘어나 최대 5분이고 ‘거절’은 재시도하지 않습니다(`tests/unit/retry-budget.test.ts`). 이날 실제로 이 재시도가 돌았는지는 확인하지 못했습니다.
